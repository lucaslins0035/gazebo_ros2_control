...
class gazebo_ros2_control::GazeboSystemPrivate {
   public:
    ...
    /// \brief whether to predict the velocity by the difference of positions
    /// in one timestep
    std::vector<bool> predict_joint_velocity_;

    /// \brief The TF representing the joint dynamics
    std::vector<gazebo_ros2_control::JointTf> joint_sys_;
};
...
void GazeboSystem::registerJoints(
    ...
    this->dataPtr->joint_sys_.resize(this->dataPtr->n_dof_);
    this->dataPtr->predict_joint_velocity_.resize(this->dataPtr->n_dof_);
    ...
    for (unsigned int j = 0; j < this->dataPtr->n_dof_; j++) {
        ...
        // Create tf
        gazebo_ros2_control::JointTf tf;

        std::deque<std::string> v;
        // Get numerator of the tf for the joint
        try {
            std::string num = hardware_info.joints[j].parameters.at("num");
            splitString(num, v, ' ');
            tf.num.resize(v.size());
            std::transform(
                v.begin(), v.end(), tf.num.begin(),
                [](const std::string& val) { return std::stod(val); });
        } catch (...) {
            tf.num = {1.0};
        }

        // Get denominator of the tf for the joint
        try {
            std::string den = hardware_info.joints[j].parameters.at("den");
            v.clear();
            splitString(den, v, ' ');
            tf.den.resize(v.size());
            std::transform(
                v.begin(), v.end(), tf.den.begin(),
                [](const std::string& val) { return std::stod(val); });
        } catch (...) {
            tf.den = {1.0};
        }
        // Setup tf for the joint
        tf.sys.setup(tf.num, tf.den);
        this->dataPtr->joint_sys_[j] = tf;

        // Get whether to predict velocity when reading position
        try {
            std::string prev =
                hardware_info.joints[j].parameters.at("predict_vel");
            if ((prev == "true") || (prev == "True")) {
                this->dataPtr->predict_joint_velocity_[j] = true;
            } else {
                this->dataPtr->predict_joint_velocity_[j] = false;
            }
        } catch (...) {
            this->dataPtr->predict_joint_velocity_[j] = false;
        }
        ...
        gazebo::physics::JointPtr simjoint =
            parent_model->GetJoint(joint_name);
        ...
        for (unsigned int i = 0;
             i < hardware_info.joints[j].state_interfaces.size(); i++) {
            if (hardware_info.joints[j].state_interfaces[i].name ==
                "position") {
                RCLCPP_INFO_STREAM(this->nh_->get_logger(), "\t\t position");
...
void GazeboSystem::splitString(std::string str, std::deque<std::string>& v,
                               const char div) {
    std::string temp = "";
...
void GazeboSystem::registerSensors(
    ...
    // associate the interfaces So we have resize only once the structures
    // where the data will be stored, and we can safely use pointers to the
    // structures
    ...
    this->dataPtr->ft_sensor_data_.resize(
    ...
    for (unsigned int i = 0; i < imu_components_.size(); i++) {
        ...
        for (const auto& state_interface :
             imu_components_[i].state_interfaces) {
...
hardware_interface::return_type GazeboSystem::configure(
    const hardware_interface::HardwareInfo& actuator_info) {
    if (configure_default(actuator_info) !=
...
hardware_interface::return_type GazeboSystem::read() {
    for (unsigned int j = 0; j < this->dataPtr->joint_names_.size(); j++) {
        if (!this->dataPtr->predict_joint_velocity_[j]) {
            ...
            if (this->dataPtr->sim_joints_[j]) {
                double vel = this->dataPtr->joint_position_[j];
                this->dataPtr->joint_position_[j] =
                    this->dataPtr->sim_joints_[j]->Position(0);
                this->dataPtr->joint_velocity_[j] =
                    (this->dataPtr->joint_position_[j] - vel) / 0.01; //TODO Remove hardcoded timestep
                this->dataPtr->joint_effort_[j] =
                    this->dataPtr->sim_joints_[j]->GetForce(0u);
            }
        }
    ...
    for (unsigned int j = 0; j < this->dataPtr->sim_imu_sensors_.size(); j++) {
        ...
        this->dataPtr->imu_sensor_data_[j][7] =
            sim_imu->LinearAcceleration().X();
        this->dataPtr->imu_sensor_data_[j][8] =
            sim_imu->LinearAcceleration().Y();
        this->dataPtr->imu_sensor_data_[j][9] =
            sim_imu->LinearAcceleration().Z();
    }
        ...
        if (this->dataPtr->sim_joints_[j]) {
            if (this->dataPtr->joint_control_methods_[j] & POSITION) {
                this->dataPtr->sim_joints_[j]->SetPosition(
                    0,
                    this->dataPtr->joint_sys_[j].sys.get_output(
                        this->dataPtr->joint_position_cmd_[j]),
                    true);
            }
...
PLUGINLIB_EXPORT_CLASS(gazebo_ros2_control::GazeboSystem,
